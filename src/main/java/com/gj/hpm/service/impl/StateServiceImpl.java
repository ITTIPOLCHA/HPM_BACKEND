...
public class StateServiceImpl implements StateService {
    ...
    public GetStateResponse getState() {
        ...
        response.setUserSent(
                (int) users.stream().filter(u -> u.getStatusFlag().equals(StatusFlag.ACTIVE.toString())).count());
        response.setUserUnsent(
                (int) users.stream().filter(u -> u.getStatusFlag().equals(StatusFlag.INACTIVE.toString())).count());
        response.setUserWarning(
                (int) users.stream().filter(u -> !u.getLevel().equals(Level.NORMAL.toString())).count());
