 ...
 * shared and re-used across multiple
 * threads.
 * </p>
 *
...
public class HmacOneTimePasswordGenerator {
     ...
     * length and algorithm. Note that
     * <a href="https://tools.ietf.org/html/rfc4226">RFC&nbsp;4226</a> specifies
     * that HOTP must always use HMAC-SHA1 as
     * an algorithm, but derived one-time password systems like TOTP may allow for
     * other algorithms.
     * </p>
     *
     * @param passwordLength the length, in decimal digits, of the one-time
     ...
     *                       use when generating passwords; note that
     *                       HOTP only allows for
     *                       {@value com.ntt.otp.otpUtil.eatthepath.otp.HmacOneTimePasswordGenerator#HOTP_HMAC_ALGORITHM},
     *                       but derived
     *                       standards like TOTP may allow for other algorithms
     *
     * @throws NoSuchAlgorithmException if the given algorithm is not supported by
     *                                  the underlying JRE
     */
    protected HmacOneTimePasswordGenerator(final int passwordLength, final String algorithm)
        ...
        // Our purpose here is just to throw an exception immediately if the algorithm
        // is bogus.
    ...
    public int generateOneTimePassword(final Key key, final long counter) throws InvalidKeyException {
        ...
        try {
            ...
            // This should never happen since we verify that the algorithm is legit in the
            // constructor.
